...
describe("#shouldBuyCar()", function () {
  ...
  // Happy Path
  it("should return true when the car has 7 litres/100km and car price is $5000 or greater", function () {
    const car = {
      litresPer100km: 7,
      price: 5000 // Edge Case
    };
    const shouldBuy = shouldBuyCar(car);
    assert.isTrue(shouldBuy);
  });
  ...
  // Sad Path
  it("should return false when the car has 5 litres/100km", function () {
    const car = {
      litresPer100km: 5, // Edge Case
    };
    const shouldBuy = shouldBuyCar(car);
    assert.isFalse(shouldBuy);
  });
